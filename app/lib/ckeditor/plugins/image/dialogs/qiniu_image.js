...
function savetoqiniu(){
	...
	init: {
		...
		"FileUploaded": function(up, file, info) {
			...
			console.info(info,res,sourceLink);
			document.getElementById(file.id).getElementsByTagName("i")[0].innerHTML = res.name;
		...
		"Key": function(up, file) {
			var key = "";
			key = Math.round(new Date().getTime() / 1000) + "_" + file.name;
			return key
		}
	}
});
